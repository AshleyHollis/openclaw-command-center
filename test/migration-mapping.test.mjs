...
import { createMigrationFixtureService, migrationFixtureFolder } from './fixtures/migration-folders.mjs';
...
test('one configured channel reconciles one provisioning Topic, Note Folder, and Primary Session', async () => {
  ...
  try {
    ...
    metadata.close();
    metadata = openCommandCenterMetadataService({ stateDir, capabilities: { notes: true, sessions: true, activity: true } });
    const restarted = createMigrationFixtureService({ metadata, config, gateway, transcriptRuntime: transcripts });
...
test('5,000 imported occurrences use one checkpoint scan and constant-size persistence results', async () => {
  ...
  try {
    ...
    const service = createMigrationFixtureService({ metadata: countedMetadata, config: scaleConfig, gateway: { request: async (_method, params) => ({ ['k' + 'ey']: params.key, sessionId: 'fictional-session-scale' }) }, transcriptRuntime: runtime() });
...
test('external plugin startup provisions through the public Session store runtime without trusted Gateway RPC', async () => {
  ...
  try {
    ...
    let admittedTarget;
    let verifying = false;
    const nativeLock = transcripts.withSessionTranscriptWriteLock;
    transcripts.withSessionTranscriptWriteLock = async (target, run) => nativeLock(target, async locked => {
      admittedTarget = target;
      // Simulate configuration drift after admission, before the public read.
      if (verifying) sessionStore.resolveStorePath = () => path.join(stateDir, 'fictional-new-store.json');
      return run(locked);
    });
    transcripts.readSessionTranscriptEvents = async target => {
      assert.equal(target.storePath, expectedStorePath);
      assert.deepEqual(target, admittedTarget, 'public verification must retain the exact lock admission target');
      boundRead = true;
      return transcripts.sessions.get(target.sessionKey);
    };
    const service = createMigrationFixtureService({ metadata, config, gateway, sessionStore, transcriptRuntime: transcripts, hooks: { afterVerify() { verifying = true; } } });

    assert.equal((await service.start()).complete, true);
    assert.equal(boundRead, true);
...
test('a foreign Session claiming the deterministic key after binding persistence is never adopted', async () => {
  ...
  try {
    ...
    await createMigrationFixtureService({ metadata, config, gateway: { request: async () => ({ sessions: [] }) }, hooks: { afterTopicBinding() { throw new Error('fictional interruption after atomic folder binding'); } } }).start();
    ...
    const service = createMigrationFixtureService({ metadata, config, sessionStore, transcriptRuntime: runtime() });
...
test('a pre-existing unowned Note Folder Source Reference is rejected without adding a Session binding', async () => {
  ...
  try {
    ...
    metadata.createSourceReference({ version: 1, referenceId: 'fictional-folder-reference', topicId: 'fictional-topic-existing-folder', sourceSystem: 'obsidian', sourceKind: 'note_folder', externalSourceId: migrationFixtureFolder(metadata, '/fictional/vault/alpha'), observedRevision: null });
    ...
    const service = createMigrationFixtureService({ metadata, config: { ...config, channels: [{ ...config.channels[0], topicId: 'fictional-topic-existing-folder' }] }, gateway, transcriptRuntime: runtime() });
...
test('a configured Note Folder owned by another Topic fails whole-set preflight', async () => {
  ...
  try {
    ...
    metadata.createSourceReference({ version: 1, referenceId: 'fictional-owned-folder', topicId: 'fictional-topic-folder-owner', sourceSystem: 'obsidian', sourceKind: 'note_folder', externalSourceId: migrationFixtureFolder(metadata, '/fictional/vault/alpha'), observedRevision: null });
    let sessionCreates = 0;
    const service = createMigrationFixtureService({ metadata, config, gateway: { request: async () => { sessionCreates += 1; return {}; } }, transcriptRuntime: runtime() });
    assert.equal((await service.start()).phase, 'review');
...
test('an explicitly mapped channel with no occurrences still completes an empty immutable prefix', async () => {
  ...
  try {
    ...
    metadata = openCommandCenterMetadataService({ stateDir, capabilities: { notes: true, sessions: true } });
    const transcripts = runtime();
    const service = createMigrationFixtureService({ metadata, config: { ...config, exportPath, channels: [{ ...config.channels[0], topicId: 'fictional-topic-empty' }] }, gateway: { request: async (_method, params) => ({ ['k' + 'ey']: params.key, sessionId: 'fictional-session-empty' }) }, transcriptRuntime: transcripts });
    assert.equal((await service.start()).complete, true);
...
test('canonical Note Folder aliases are rejected before creating Topics or Sessions', async () => {
  ...
  try {
    ...
    metadata = openCommandCenterMetadataService({ stateDir, capabilities: { notes: true, sessions: true } });
    let sessionCreates = 0;
    const service = createMigrationFixtureService({
      metadata,
      config: {
        ...config,
