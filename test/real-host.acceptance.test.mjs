...
async function verifyReleaseSearchResults({ gatewayUrl, credential, projectionRoot, signal }) {
  ...
  return waitForCommittedSearchProjections(projectionRoot, {
    ...
    expectedTopicRowCounts: {
      ...
      conversationMessages: { [RELEASE_SCALE_TOPIC_ID]: RELEASE_FIXTURE_COUNTS.indexedConversationMessages }
    }
  });
}
...
async function exerciseRecoveryOnlyHostVariant({ descriptor, buildReceipt, signal }) {
  return withIsolatedWorld(async (recoveryWorld) => {
    ...
    try {
      ...
      assert.equal(releasePerformanceIdentity.hostReceipt.commit, 'e686a7e7963abedd5e5fa14561a2d3c71692c790', 'the launched runtime must match the exact authenticated compatibility tuple');
...
async function exerciseSecureHostVariant({ descriptor, buildReceipt, signal, onFinalization }) {
  return withIsolatedWorld(async (secureWorld) => {
    ...
    try {
      managedBrowser = await withDeadline('secure-origin browser launch', () => launchManagedBrowser({ headless: true, timeout: 60_000, args: [`--host-resolver-rules=MAP ${fictionalTailnetHost} 127.0.0.1`, '--disable-features=AsyncDns'] }));
      browser = managedBrowser.browser;
      const context = await browser.newContext({ ignoreHTTPSErrors: true, viewport: { width: 1440, height: 900 } });
      const page = await context.newPage();
      await configureEvidencePage(page, secureBrowserGuard, evidence, {
        destinationForRequest: request => new URL(request.url()).hostname === fictionalTailnetHost ? '127.0.0.1' : new URL(request.url()).hostname
      });
      await page.route('**/*', async (route) => {
        ...
        const loopbackUrl = new URL(url.href);
        loopbackUrl.hostname = '127.0.0.1';
        try {
          const response = await route.fetch({ url: loopbackUrl.href, headers: { ...route.request().headers(), host: fictionalTailnetHost } });
      ...
      await page.routeWebSocket('**/*', socket => {
        ...
        const server = new NodeWebSocket(target.href, { rejectUnauthorized: false, headers: { host: fictionalTailnetHost, origin: secureUrl } });
        const requests = new Map();
        const pending = [];
        const forward = payload => {
          if (server.readyState === NodeWebSocket.OPEN) server.send(String(payload));
          else if (pending.length < 32) pending.push(payload);
          else recordBounded(evidence.errors, 'Secure native observation capacity exceeded');
        ...
        server.on('open', () => { for (const payload of pending.splice(0)) server.send(String(payload)); });
        server.on('message', payload => {
          socket.send(String(payload));
          let frame; try { frame = JSON.parse(String(payload)); } catch { return; }
        ...
        server.on('error', error => { recordBounded(evidence.errors, redactBrowserEvidence(error.message)); void socket.close(); });
  ...
  return withIsolatedWorld(async (variantWorld) => {
    ...
    try {
      if (kind === 'plugin-api') {
        ...
        const escapedHostVersion = releasePerformanceIdentity.hostVersion.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        assert.match(`${variantHost.diagnostics.stdout}\n${variantHost.diagnostics.stderr}`, new RegExp(`plugin requires plugin API =1900\\.1\\.1, but this host is ${escapedHostVersion}; skipping (?:discovery|load)`, 'u'));
      ...
      if (kind === 'scale') {
        ...
        const verifyScaleSearch = async () => {
          ...
          await waitForCommittedSearchProjections(scaleProjectionRoot, { requiredTopicIds: [scaleTopicId], expectedTopicRowCounts: {
            ...
            conversationMessages: { [scaleTopicId]: RELEASE_FIXTURE_COUNTS.indexedConversationMessages },
            conversationMetadata: { [scaleTopicId]: RELEASE_FIXTURE_COUNTS.conversations - 1 },
            conversations: { [scaleTopicId]: RELEASE_FIXTURE_COUNTS.indexedConversationMessages + RELEASE_FIXTURE_COUNTS.conversations - 1 }
          } });
        };
...
async function runUiJourney(frame, { page, width, name, category = 'project', keyboard = false, projectionRoot } = {}) {
  ...
  await timed(() => closeOpenConversation(conversation, { activate, keyboard, timeout: BRIDGE_UI_OPERATION_BUDGET_MS }));
...
test('mounts the built plugin through the isolated authenticated external tab', { timeout: 900_000, concurrency: true }, async (testContext) => {
  ...
  if (nativeDiagnostic) {
    ...
    if (capturePerformanceBaseline) {
      assert.equal(nativeDiagnostic, 'scale-performance');
      assert.deepEqual(evidence.fixtureCounts, RELEASE_FIXTURE_COUNTS);
      capturedBaseline = captureFirstReleasePerformanceBaseline({
        ...
        fixtureCounts: evidence.fixtureCounts,
        capture: { policy: 'first-successful-pinned-harness-observation', successfulRunOrdinal: null }
      }, evidence.observations);
      await writeFile(capturedPerformanceBaselinePath, `${JSON.stringify(capturedBaseline, null, 2)}\n`, { flag: 'wx' });
    }
    testContext.diagnostic(`acceptance-scenario-result=${JSON.stringify({ schemaVersion: 1, outcome: 'passed', scenario: nativeDiagnostic, scenarioIds: acceptancePlan.scenarioIds, buildDigest: buildReceipt.digest, performanceQualified: Boolean(capturedBaseline), performanceBaseline: capturedBaseline, evidence })}`);
    return;
  }
  if (['release', 'prerequisites'].includes(acceptancePlan.kind)) {
    assert.ok(descriptor && buildReceipt, 'Native release requires successful descriptor and sealed receipt admission');
    const prerequisitesOnly = acceptancePlan.kind === 'prerequisites';
    ...
    const nativeResult = await execute({
      ...
      runners: {
        ...
        ...(prerequisitesOnly ? {} : { scale: bind(exerciseNativeScaleJourney) })
      },
    ...
    if (prerequisitesOnly) {
      scanPublicEvidence([JSON.stringify(nativeResult)]);
      testContext.diagnostic(`acceptance-prerequisites=${JSON.stringify(nativeResult)}`);
      return;
    }
    const { report, capturedBaseline } = nativeResult;
    const result = {
      ...
      await collectScenario('focused-control-ui-search-projection', async (signal) => {
        ...
        const verified = await waitForCommittedSearchProjections(projectionRoot, {
          ...
          ...(heavyCorpus ? {
            expectedTopicRowCounts: {
              notes: { [RELEASE_SCALE_TOPIC_ID]: RELEASE_FIXTURE_COUNTS.indexedNotes },
              conversationMessages: { [RELEASE_SCALE_TOPIC_ID]: fullCorpus ? RELEASE_FIXTURE_COUNTS.indexedConversationMessages : 1 }
            }
          } : {})
        });
        releaseState.projectionRoot = projectionRoot;
