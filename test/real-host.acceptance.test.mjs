...
async function exerciseRecoveryOnlyHostVariant({ descriptor, buildReceipt, signal }) {
  return withIsolatedWorld(async (recoveryWorld) => {
    ...
    try {
      ...
      assert.equal(releasePerformanceIdentity.hostReceipt.commit, 'db18951ddcd90fbe04f83c9709a95d9797a723b2', 'the launched runtime must match the exact authenticated compatibility tuple');
