...
test('public source service writes durable authoritative Markdown and keeps metadata free of content', async () => {
  ...
  try {
    ...
    const compressible = await service.notesCreate({ schemaVersion: 1, topicId: 'topic-integration', path: 'nested/compressible.md', text: 'z'.repeat(2 * 1024 * 1024) });
    const compressed = await service.notesRead({ schemaVersion: 1, topicId: 'topic-integration', referenceId: compressible.note.sourceReference.referenceId, path: 'nested/compressible.md', offset: 0 });
