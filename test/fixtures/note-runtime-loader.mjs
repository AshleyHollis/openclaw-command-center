...
if (process.env.COMMAND_CENTER_TEST_SQLITE_RUNTIME || process.env.COMMAND_CENTER_TEST_FILE_ACCESS_RUNTIME || process.env.COMMAND_CENTER_TEST_SESSION_STORE_RUNTIME || process.env.COMMAND_CENTER_TEST_SESSION_TRANSCRIPT_RUNTIME) registerHooks({
  resolve(specifier, context, nextResolve) {
    ...
    if (specifier === 'openclaw/plugin-sdk/session-transcript-runtime' && process.env.COMMAND_CENTER_TEST_SESSION_TRANSCRIPT_RUNTIME) return { url: process.env.COMMAND_CENTER_TEST_SESSION_TRANSCRIPT_RUNTIME, shortCircuit: true };
