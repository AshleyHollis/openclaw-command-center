...
test('release performance seed pins the exact corpus without claiming an unobserved successful baseline', async () => {
  ...
  assert.deepEqual(seed.fixtureCounts, { chunkBoundaryNoteBytes: 524289, conversations: 50, activityRecords: 51, actionCards: 2, indexedNotes: 5000, indexedConversationMessages: 5000 });
...
test('release performance baseline rejects receipt drift, widened ceilings, zero observations, and silent regeneration', async () => {
  ...
  assert.throws(() => validateReleasePerformanceBaseline({ ...baseline, fixtureCounts: { ...baseline.fixtureCounts, conversations: 49 } }), /conversations must be 50/);
...
test('release fixture includes one real multibyte Note across the UTF-8 read chunk', () => {
