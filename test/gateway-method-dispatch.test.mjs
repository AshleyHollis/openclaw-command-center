...
test('Conversation authority refuses missing identity or lifetime and detects changed admitted grants', async () => {
  ...
  const connectionOnly = makeScope();
  delete connectionOnly.client.authenticatedUserProfile;
  connectionOnly.client.connId = 'fictional-connection';
  await assert.rejects(() => requestRuntime.createRequestScopedConversationRuntime({ getRequestScope: () => connectionOnly }), { code: 'unauthenticated' });
...
test('request-scoped runtime can use the published plugin-owned Gateway facade', async () => {
  ...
  const runtime = await requestRuntime.createRequestScopedConversationRuntime({
    ...
    gatewayRequest: async (...args) => { calls.push(args); return { key: 'fictional-session' }; }
  });
  const logicalOperationId = '11111111-1111-4111-8111-111111111111';
  const result = await runtime.gatewayRequest('sessions.create', {
    agentId: 'main', label: 'Fictional Conversation', idempotencyKey: logicalOperationId
  }, { requestId: logicalOperationId });
  assert.deepEqual(result, { key: 'fictional-session' });
  assert.deepEqual(calls, [['sessions.create', {
    agentId: 'main', label: 'Fictional Conversation', idempotencyKey: logicalOperationId, catalogId: 'command-center'
  }, { requestId: logicalOperationId }]]);
});
...
test('request-scoped Session dispatch preserves the pinned create envelope and idempotency identity', async () => {
  ...
  assert.deepEqual(calls, [['sessions.create', { ...params, catalogId: 'command-center' }, { expectFinal: true, timeoutMs: 45_000 }]]);
...
test('production Session adapter recovers interrupted native catalog readback without creating another Session', async () => {
  ...
  assert.deepEqual(calls[0].params, { agentId: 'main', label: 'Pinned Envelope', idempotencyKey: logicalOperationId, catalogId: 'command-center' });
