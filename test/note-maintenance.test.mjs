...
test('automatic Note maintenance uses guarded CAS and one deduplicated Activity without push or Attention', async () => {
  ...
  try {
    ...
    const folder = { version: 1, referenceId: 'folder:maintenance', topicId: 'topic-maintenance', sourceSystem: 'obsidian', sourceKind: 'note_folder', externalSourceId: root };
    const folderRevision = await enrollNoteFolderIdentity(root);
    const references = [folder];
    const metadata = {
      ...
      getSourceLocator: (referenceId) => referenceId === folder.referenceId ? { referenceId, locator: root, locatorVersion: 1, observedRevision: folderRevision } : null,
    ...
    notes = new NoteAdapter({ fsSafeRootFactory, metadata, topicId: 'topic-maintenance', noteFolderReferenceId: folder.referenceId });
  ...
  } finally { notes?.close(); await rm(root, { recursive: true, force: true }); }
});
