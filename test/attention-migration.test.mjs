...
test('schema-2 metadata migrates forward to the Attention schema and reopens with durable tables', async () => {
  ...
  try {
    ...
    assert.equal(inspectSchema(check, 9).valid, true);
...
test('schema-3 metadata migrates narrowly to schema 6 without losing migration state', async () => {
  ...
  try {
    ...
    assert.deepEqual(check.prepare('SELECT from_version, to_version FROM schema_migrations').all().map((row) => ({ ...row })), [{ from_version: 3, to_version: 4 }, { from_version: 4, to_version: 5 }, { from_version: 5, to_version: 6 }, { from_version: 6, to_version: 7 }, { from_version: 7, to_version: 8 }, { from_version: 8, to_version: 9 }]);
    check.close();
  } finally { await rm(stateDir, { recursive: true, force: true }); }
});
