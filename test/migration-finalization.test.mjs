...
for (const failure of ['read-fence', 'foreign-prefix']) {
  test(`public native verification ${failure} prevents activation without falling back to an unfenced read`, async () => {
    const stateDir = await mkdtemp(path.join(os.tmpdir(), 'command-center-migration-public-read-'));
    let metadata;
    try {
      metadata = openCommandCenterMetadataService({ stateDir, capabilities: { notes: true, sessions: true } });
      const transcripts = runtime();
      transcripts.readSessionTranscriptEvents = async ({ sessionKey }) => {
        if (failure === 'read-fence') throw Object.assign(new Error('Fictional native read fence'), { code: 'fictional-read-fenced' });
        return [{ id: 'fictional-foreign-prefix', message: { role: 'user', content: 'Foreign event' } }, ...transcripts.sessions.get(sessionKey)];
      };
      const service = createMigrationFixtureService({ metadata,
        config: { schemaVersion: 1, exportPath: fixturePath, channels: [channel] },
        gateway: { request: async (_method, params) => ({ key: params.key, sessionId: 'fictional-public-read' }) },
        transcriptRuntime: transcripts });
      ...
      assert.equal([...transcripts.sessions.values()][0].length, 2);
...
test('destination corruption blocks activation and completion; success leaves only a payload-free tombstone', async () => {
  ...
  try {
    metadata = openCommandCenterMetadataService({ stateDir, capabilities: { notes: true, sessions: true } });
    const transcripts = runtime();
    const service = createMigrationFixtureService({ metadata, config: { schemaVersion: 1, exportPath: fixturePath, channels: [channel] }, gateway: { request: async (_method, params) => ({ ['k' + 'ey']: params.key, sessionId: 'fictional-finalization' }) }, transcriptRuntime: transcripts, hooks: { afterCheckpoint({ displayOrder }) { if (displayOrder === 1) { const events = [...transcripts.sessions.values()][0]; events.push({ id: 'foreign-during-bootstrap', parentId: events.at(-1).id, message: { role: 'user', text: 'fictional foreign event' } }); } } } });
    ...
    try {
      ...
      const ambiguousService = createMigrationFixtureService({
    ...
    try {
      ...
      const racedService = createMigrationFixtureService({ metadata: racedMetadata, config: { schemaVersion: 1, exportPath: fixturePath, channels: [{ ...channel, topicId: racedTopicId }] }, gateway: { request: async (_method, params) => ({ ['k' + 'ey']: params.key, sessionId: 'fictional-transcript-race' }) }, transcriptRuntime: racedRuntime, hooks: { afterVerify() { const events = [...racedRuntime.sessions.values()][0]; events.push({ id: 'foreign-after-verification', parentId: events.at(-1).id, message: { role: 'user', text: 'fictional race' } }); } } });
    ...
    try {
      ...
      const cleanService = createMigrationFixtureService({ metadata: cleanMetadata, config: cleanConfig, gateway: { request: async (_method, params) => ({ ['k' + 'ey']: params.key, sessionId: 'fictional-finalization-clean' }) }, transcriptRuntime: cleanRuntime });
      ...
      const restarted = createMigrationFixtureService({ metadata: cleanMetadata });
      ...
      const malformedRestart = createMigrationFixtureService({ metadata: cleanMetadata, config: { schemaVersion: 999, unexpected: true } });
      assert.equal((await malformedRestart.start()).phase, 'complete');
      const changedRestart = createMigrationFixtureService({ metadata: cleanMetadata, config: { schemaVersion: 1, exportPath: fixturePath, channels: [{ ...channel, topicId: 'fictional-topic-conflicting-completion' }] } });
      ...
      const unchangedRestart = createMigrationFixtureService({ metadata: cleanMetadata, config: cleanConfig, transcriptRuntime: cleanRuntime });
      ...
      const replacedPrimaryRestart = createMigrationFixtureService({ metadata: cleanMetadata });
      ...
      const missingBindingRestart = createMigrationFixtureService({ metadata: cleanMetadata });
