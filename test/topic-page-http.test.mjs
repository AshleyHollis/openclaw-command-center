...
test('a migrated canonical scale Topic creates 99 Conversations through the public route and authoritatively totals 100', async () => {
  ...
  try {
  const scaleFolder = path.join(stateDir, 'fictional-vault', 'scale');
  await mkdir(scaleFolder, { recursive: true });
  const service = fixtureService();
  ...
  const migration = createLegacyDiscordMigrationService({
    ...
    config: {
      ...
      channels: [{ channelId: 'fictional-channel-alpha', topicId: scaleTopicId, paraCategory: 'resource', noteFolderPath: scaleFolder }]
    },
