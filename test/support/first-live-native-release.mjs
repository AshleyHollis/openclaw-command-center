...
import { createAcceptanceReport, assertAcceptanceReportPassed, FINALIZATION_PHASES, RELEASE_ROW_IDS } from '../../src/acceptance-report.mjs';
 ...
 */
export async function runNativeReleaseCapture({ buildReceipt, descriptor, runners, capturePerformanceBaseline = false, baseline,
  scanArtifacts, onProgress = () => {}, timeoutMs = 240_000, cleanupTimeoutMs = 15_000 } = {}) {
  ...
  assert.deepEqual(Object.keys(runners ?? {}).sort(), [...PARTICIPANTS].sort(), 'All fifteen closed native participants are required');
  ...
  const existingBaseline = capturePerformanceBaseline ? undefined : validateReleasePerformanceBaseline(structuredClone(baseline));
