...
import { controlUiPluginUrl, isCommandCenterMetadataReady } from '../../src/acceptance-readiness.mjs';
...
export async function exerciseNativeJourney({ descriptor, buildReceipt, signal, keyboard = false, scale = false, onFinalization }) {
  ...
  return withIsolatedWorld(async (world) => {
    ...
    try {
      ...
      assert.equal(hasSuccessfulBrowserResponse(observedCreation), true, 'The native form must submit the authenticated real Conversation action');
