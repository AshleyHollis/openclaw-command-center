...
export async function startFictionalOpenAiModel({ firstTurnFinal = false } = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (accountedPhaseOne && accounted.phaseOneStep <= 4) {
    ...
    } else if (accountedPhaseTwo && accounted.phaseTwoStep <= 9) {
      ...
      if (step === 0 || step === 9) { action = step === 0 ? 'accounted-load' : 'accounted-load-final'; frames = toolCall({ id, model, name: 'command_center_get_intake_source_account', arguments: source }); }
      else if (step === 1) { action = 'accounted-resolve'; frames = toolCall({ id, model, name: 'command_center_resolve_source_topic', arguments: { topicName: durableExtraction?.proposedTopic } }); }
      else if (step === 2) { action = 'accounted-save'; frames = toolCall({ id, model, name: 'command_center_save_source_note', arguments: { topicId: accounted.resolved?.topicId, noteFolderReferenceId: accounted.resolved?.noteFolderReferenceId, ...source, path: durableExtraction?.notePath, markdown: durableExtraction?.knowledgeMarkdown } }); }
      else if (step === 3) { const outcome = durableOutcome('real-host-reference'); action = 'accounted-outcome-reference'; frames = toolCall({ id, model, name: 'command_center_record_intake_outcome', arguments: { ...source, outcomeId: outcome?.outcomeId, kind: outcome?.kind, status: 'quiet', summary: 'Retain fictional real-host reference', topicId: accounted.resolved?.topicId, sourceReferenceId: accounted.saved?.sourceReferenceId, sourcePath: accounted.saved?.path, sourceReferenceVersion: accounted.saved?.revision, recordedAt: '2026-09-22T04:01:00.000Z' } }); }
      else if (step === 4 || step === 6) { const payment = step === 4; const obligation = durableExtraction?.obligations?.find(item => item.obligationId === (payment ? 'real-host-payment' : 'real-host-reply')); action = payment ? 'accounted-capture-payment' : 'accounted-capture-reply'; frames = toolCall({ id, model, name: 'command_center_capture_source_commitment', arguments: { ...obligation, classification: undefined, topicId: accounted.resolved?.topicId, ...source, sourceReferenceId: accounted.saved?.sourceReferenceId, sourcePath: accounted.saved?.path } }); }
      else if (step === 5 || step === 7) { const payment = step === 5; const outcome = durableOutcome(payment ? 'real-host-payment' : 'real-host-reply'); const obligation = durableExtraction?.obligations?.find(item => item.obligationId === outcome?.outcomeId); action = payment ? 'accounted-outcome-payment' : 'accounted-outcome-reply'; frames = toolCall({ id, model, name: 'command_center_record_intake_outcome', arguments: { ...source, outcomeId: outcome?.outcomeId, kind: outcome?.kind, status: 'applied', summary: obligation?.title, loopId: accounted.captured?.loopId, recordedAt: payment ? '2026-09-22T04:01:10.000Z' : '2026-09-22T04:01:20.000Z' } }); }
