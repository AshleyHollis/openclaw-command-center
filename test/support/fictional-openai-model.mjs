...
export async function startFictionalOpenAiModel({ firstTurnFinal = false } = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (accountedPhaseOne && accounted.phaseOneStep <= 4) {
      ...
      else if (step === 1) { action = 'accounted-resolve'; frames = toolCall({ id, model, name: 'command_center_resolve_source_topic', arguments: { topicName: acceptedExtraction.proposedTopic } }); }
      else if (step === 2) { action = 'accounted-save'; frames = toolCall({ id, model, name: 'command_center_save_source_note', arguments: { topicId: accounted.resolved?.topicId, noteFolderReferenceId: accounted.resolved?.noteFolderReferenceId, ...source, path: acceptedExtraction.notePath, markdown: acceptedExtraction.knowledgeMarkdown } }); }
      ...
      else if (step === 4 || step === 6) { const payment = step === 4; action = payment ? 'accounted-capture-payment' : 'accounted-capture-reply'; frames = toolCall({ id, model, name: 'command_center_capture_source_commitment', arguments: { topicId: accounted.resolved?.topicId, ...source, sourceReferenceId: accounted.saved?.sourceReferenceId, sourcePath: accounted.saved?.path, title: payment ? 'Pay fictional real-host invoice' : 'Reply with fictional real-host reference', obligationId: payment ? 'real-host-payment' : 'real-host-reply', provenance: 'explicit' } }); }
      else if (step === 5 || step === 7) { const payment = step === 5; action = payment ? 'accounted-outcome-payment' : 'accounted-outcome-reply'; frames = toolCall({ id, model, name: 'command_center_record_intake_outcome', arguments: { ...source, outcomeId: payment ? 'real-host-payment' : 'real-host-reply', kind: 'obligation', status: 'applied', summary: payment ? 'Pay fictional real-host invoice' : 'Reply with fictional real-host reference', loopId: accounted.captured?.loopId, recordedAt: payment ? '2026-09-22T04:01:10.000Z' : '2026-09-22T04:01:20.000Z' } }); }
