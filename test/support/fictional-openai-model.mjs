...
export async function startFictionalOpenAiModel({ firstTurnFinal = false } = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (accountedPhaseOne && accounted.phaseOneStep <= 6) {
      ...
      else { const plan = fictionalAccountedEmailPlan(accounted.saved?.revision); action = 'accounted-receipt-pending'; frames = toolCall({ id, model, name: 'command_center_record_intake_receipt', arguments: { sourceKind: 'email', runId: plan.runId, planDigest: producerIntakePlanDigest(plan), checkpoint: 'start', status: 'pending', observedAt: '2026-09-22T04:00:11.000Z', nextExpectedAt: plan.nextExpectedAt, processedCount: 0, actionableCount: 0, noteCount: 0, scope: plan.scope, enumeration: plan.enumeration } }); }
