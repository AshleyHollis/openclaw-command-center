...
test('large repeated Topic queries use the FTS virtual-table index without authoritative reads', async () => {
  ...
  try {
    ...
    await conversationStore.rebuild({ rows: Array.from({ length: 5_000 }, (_, index) => ({
      ...
      closed: false, primaryState: 'ordinary', role: 'user', text: index % 100 === 0 ? `indexed needle ${index}` : `ordinary fictional message ${index}`, provenance: 'native'
    })) });
