...
test('recovery-only startup preserves refused storage and serves safe status without starting writable consumers', async () => {
  ...
  try {
    ...
    await assert.rejects(service.dashboardGet({ schemaVersion: 1 }), (error) => error.code === 'recovery-only');
    assert.throws(() => service.topicAnalysisRead(), (error) => error.code === 'recovery-only');
