...
test('historical backfill CLI runs a digest-pinned private adapter with durable metadata', async t => {
  ...
  const adapterSource = `export function createHistoricalBackfillAdapter() { return {
    ...
    async classify() { return { schemaVersion: 1, disposition: 'knowledge-only' }; },
    async applyRecord() { throw new Error('preview must not apply'); }, async reconcileRecord() { return { status: 'not-applied' }; }, async recordReceipt() {}
  }; }\n`;
  ...
  try {
    ...
    assert.equal(result.complete, true); assert.equal(result.counts.knowledgeOnly, 1);
