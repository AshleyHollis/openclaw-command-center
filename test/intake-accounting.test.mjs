...
test('mixed email accounting distinguishes accounted-for from resolved and retains bounded enumeration gaps', async () => {
  ...
  try {
    const metadata = openCommandCenterMetadataService({ stateDir: temporary.path }); addTopic(metadata);
    ...
    recordIntakeOutcome(metadata, { ...base, outcomeId: 'pay-invoice', kind: 'obligation', status: 'applied', summary: 'Pay fictional invoice', loopId: 'loop:payment-fictional' });
    recordIntakeOutcome(metadata, { ...base, outcomeId: 'send-reference', kind: 'obligation', status: 'applied', summary: 'Send fictional reference', loopId: 'loop:response-fictional' });
    ...
    recordIntakeOutcome(metadata, { ...base, outcomeId: 'reference-details', kind: 'information', status: 'quiet', summary: 'Retained fictional reference details', sourceReferenceId: 'note:fictional-message-42' });
    ...
    assert.equal(recordIntakeOutcome(metadata, { ...base, outcomeId: 'reference-details', kind: 'information', status: 'quiet', summary: 'Retained fictional reference details', sourceReferenceId: 'note:fictional-message-42' }).disposition, 'duplicate');
    assert.throws(() => recordIntakeOutcome(metadata, { ...base, outcomeId: 'reference-details', kind: 'information', status: 'quiet', summary: 'Changed under retry', sourceReferenceId: 'note:fictional-message-42' }), { code: 'intent-mismatch' });
    metadata.close();
...
test('a structured clarification resolves only its linked outcome and survives SQLite restart', async () => {
  ...
  try {
    let metadata = openCommandCenterMetadataService({ stateDir: temporary.path }); addTopic(metadata); recordIntakeSourcePlan(metadata, sourcePlan()); const decision = addDecisionLoop(metadata);
    const base = { schemaVersion: 1, sourceKind: 'email', sourceExternalId: 'fictional-message-42', sourceVersion: 'change-key-7', recordedAt: '2026-09-22T01:01:00.000Z' };
    recordIntakeOutcome(metadata, { ...base, outcomeId: 'pay-invoice', kind: 'obligation', status: 'applied', summary: 'Pay fictional invoice', loopId: 'loop:payment-fictional' });
    recordIntakeOutcome(metadata, { ...base, outcomeId: 'send-reference', kind: 'obligation', status: 'applied', summary: 'Send fictional reference', loopId: 'loop:response-fictional' });
    recordIntakeOutcome(metadata, { ...base, outcomeId: 'choose-delivery', kind: 'decision', status: 'pending-decision', summary: 'Choose fictional delivery window', loopId: decision.loopId });
