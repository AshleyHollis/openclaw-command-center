...
test('production build packages the complete persistence runtime graph without a new SQLite dependency', async () => {
  await build();
  for (const filename of ['compatibility.mjs', 'persistence/service.mjs', 'persistence/migrations.mjs', 'persistence/validation.mjs', 'persistence/schema.mjs']) await access(new URL(filename, `file://${distRoot}/`));
