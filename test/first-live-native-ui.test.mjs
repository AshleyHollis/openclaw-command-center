      ...
      const host = {
        ...
        async httpRequest(request, { signal }) {
          ...
          return { status: 200, body: JSON.stringify({ schemaVersion: 1, status: 'applied', logicalOperationId: body.logicalOperationId, result: {
            action: window.wrongCreationAction ? 'conversations.creation.acknowledge' : body.action,
            topicId: window.wrongCreationTopic ? 'fictional-foreign-topic' : body.topicId,
...
for (const mismatch of ['Action', 'Topic']) test(`a forged native creation ${mismatch.toLowerCase()} is retained as unknown without opening a Session`, { timeout: 30000 }, () => fixture(async page => {
  await page.evaluate(mismatch => { window[`wrongCreation${mismatch}`] = true; }, mismatch);
  await page.getByRole('button', { name: 'Create Conversation', exact: true }).click();
  await page.getByText(/Conversation creation outcome is unknown/).waitFor();
  assert.deepEqual(await page.evaluate(() => window.opened), []);
  assert.equal(await page.getByRole('button', { name: 'Create Conversation', exact: true }).isDisabled(), true);
  const posted = await page.evaluate(() => window.posts[1].body);
  assert.equal(posted.action, 'conversations.create');
  assert.equal(posted.topicId, '11111111-1111-4111-8111-111111111111');
  assert.match(await page.getByRole('status').filter({ hasText: 'Conversation creation outcome' }).textContent(), /outcome is unknown/i);
}, { start: 'topic' }));
