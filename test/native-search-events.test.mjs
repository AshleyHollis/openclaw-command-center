...
test.after(() => { delete globalThis[readerKey]; });
...
async function fixture(run, { deferredGateway = false } = {}) {
  ...
  const folderIdentity = await enrollNoteFolderIdentity(folder);
  ...
  try {
    ...
    seed.setSourceLocator({ referenceId: 'folder:fictional-search', locator: folder, observedRevision: folderIdentity, ownership: 'external' });
    ...
    const otherFolderIdentity = await enrollNoteFolderIdentity(otherFolder);
    ...
    seed.setSourceLocator({ referenceId: 'other-folder', locator: otherFolder, observedRevision: otherFolderIdentity, ownership: 'external' });
  ...
  const createDeferredSearchOwner = () => {
    let metadata;
    let sourceService;
    let searchService;
    let unsubscribe;
    ...
    return {
      async start() {
        metadata = openCommandCenterMetadataService({ stateDir, capabilities: { notes: true, sessions: true, search: true } });
        activation = new AbortController();
        const searchProvider = {};
        sourceService = createAuthoritativeSourceService({
          metadata,
          api,
          capabilities: { notes: true, sessions: true, search: true },
          searchProvider,
          transcriptReader: input => globalThis[readerKey](input)
        });
        const rebuildOwner = createSearchRebuildService({
          stateDir,
          metadata,
          api,
          transcriptReader: input => globalThis[readerKey](input),
          noteAdapterFactory: topic => sourceService.forTopic(topic).notes
        });
        searchService = createTopicSearchService({ stateDir, metadata, sourceService, rebuild: input => rebuildOwner.rebuild(input) });
        Object.assign(searchProvider, {
          rebuild: input => searchService.rebuild(input),
          reconcile: input => searchService.reconcile(input),
          invalidate: input => searchService.invalidate(input)
        });
        unsubscribe = api.runtime.events.onSessionTranscriptUpdate(update => {
          void sourceService.sessionTranscriptUpdated(update, { signal: activation.signal }).catch(() => {});
        });
        reconciliation = searchService.reconcile({ signal: activation.signal }).catch(() => {});
      },
      async stop() {
        if (!metadata) return;
        unsubscribe?.();
        ...
        await sourceService.settleSearchRefresh();
        sourceService.close();
        metadata.close();
        metadata = undefined;
      },
  ...
  let service = createDeferredSearchOwner();
  ...
  try {
    ...
    await run({ get service() { return service; }, stateDir, transcripts, controls, listeners, emit, query,
      async restart() { await service.stop(); service = createDeferredSearchOwner(); await service.start(); }
    });
