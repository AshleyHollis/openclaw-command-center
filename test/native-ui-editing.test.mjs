...
async function fixture(run) {
  ...
  try {
    ...
    await page.evaluate(async () => {
      ...
      window.posts = []; window.opened = []; window.mode = 'success';
      ...
      const host = {
        ...
        httpRequest: async (request, { signal }) => {
          ...
          else result = { action: body.action, topicId: body.topicId, referenceId: 'session:new' };
...
test('uncertain Note creation checks the original operation without replay and retains newer local text across remount', { timeout: 30000 }, () => fixture(async (page) => {
...
test('confirmed absent Note creation retains input and requires an explicit new submission', { timeout: 30000 }, () => fixture(async (page) => {
...
test('a Note creation receipt returning the Folder ID cannot acknowledge creation', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('textbox', { name: 'New Note path (required)' }).fill('new.md');
...
for (const interruption of ['permission', 'topic-switch', 'activation']) test(`a late Note creation response cannot acknowledge ${interruption} cancellation`, { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('textbox', { name: 'New Note path (required)' }).fill('new.md');
...
test('Note creation requires the exact Folder and current active Topic write policy', { timeout: 30000 }, () => fixture(async (page) => {
...
test('a superseded Note creation outcome check cannot replace a newer exact receipt', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('textbox', { name: 'New Note path (required)' }).fill('new.md');
  await page.evaluate(() => { window.mode = 'unknown'; });
  await page.getByRole('button', { name: 'Create Note', exact: true }).click();
  await page.getByText(/Note creation outcome is unknown/).waitFor();
...
test('an uncertain save has an explicit reconcile-only action preserving exact intent and newer edits', { timeout: 30000 }, () => fixture(async (page) => {
...
test('confirmed not-applied restores editing without silently retrying the save', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
...
test('native Note saves use the authoritative revision and preserve edits typed during a save', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
  const editor = page.getByRole('textbox', { name: 'Note draft' });
...
test('native creation forms use domain HTTP and open the exact newly created Conversation in Chat', { timeout: 30000 }, () => fixture(async (page) => {
  ...
  await page.waitForFunction(() => window.posts.length === 2);
  const posts = await page.evaluate(() => window.posts);
  assert.equal(posts[0].body.action, 'conversations.create');
  assert.equal(posts[0].body.expectedRevision, 4);
  assert.equal(posts[0].body.authoritativeSession, undefined);
  assert.equal(posts[1].path, '/plugins/command-center/api/topics/actions');
  assert.equal(posts[1].body.name, 'New fictional Topic');
  assert.equal(posts[1].body.paraCategory, 'area');
  assert.match(posts[1].body.topicId, /^[0-9a-f-]{36}$/);
  assert.notEqual(posts[1].body.topicId, posts[1].body.logicalOperationId);
}));
...
test('a hidden creation cannot navigate after a delayed receipt, even when shown again', { timeout: 30000 }, () => fixture(async (page) => {
...
test('unknown Note saves retain the exact draft across remount and authoritative reload without retrying', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
  const editor = page.getByRole('textbox', { name: 'Note draft' });
...
test('a conflicting draft keeps its base revision until explicit discard and never transfers to another Note', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
  const editor = page.getByRole('textbox', { name: 'Note draft' });
...
test('the latest authoritative reload owns the same Note even if the older catalog arrives first', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
  await page.evaluate(() => { window.delayBrowse = true; window.notes[0].text = 'Older version'; window.notes[0].revision = 'r2'; });
  await page.getByRole('button', { name: 'Reload authoritative Note', exact: true }).click();
  await page.waitForFunction(() => window.finishBrowse);
  await page.evaluate(() => { window.delayBrowse = true; window.notes[0].text = 'Newest version'; window.notes[0].revision = 'r3'; });
  await page.getByRole('button', { name: 'Reload authoritative Note', exact: true }).click();
  await page.waitForFunction(() => window.finishBrowses.length === 2);
  await page.evaluate(async () => { window.finishBrowses[0](); await new Promise((resolve) => setTimeout(resolve, 0)); window.finishBrowses[1](); await new Promise((resolve) => setTimeout(resolve, 0)); });
  assert.equal(await page.getByRole('region', { name: 'Note content' }).innerText(), 'Newest version');
}));

test('losing write access cancels an in-flight Note write and retains an unknown outcome', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
...
test('a late save response cannot acknowledge a revoked view even when transport ignores abort', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
  await page.getByRole('textbox', { name: 'Note draft' }).fill('Interrupted draft');
  await page.evaluate(() => { window.mode = 'delay'; window.ignoreAbort = true; });
  await page.getByRole('button', { name: 'Save Note', exact: true }).click();
...
test('a replacement activation never receives an old save acknowledgement or its draft', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
...
test('a superseded reconcile response cannot overwrite the current operation result', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
...
test('a multi-megabyte UTF-8 Note uses the bounded HTTP body and remains editable', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
...
test('keyboard save returns focus to Save Note after a refused write', { timeout: 30000 }, () => fixture(async (page) => {
  await page.getByRole('button', { name: 'Read brief.md' }).click();
