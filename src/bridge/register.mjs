...
const handlerMap = Object.freeze({
  ...
  'command-center.v1.reminders.snooze': async (service, params, runtime) => { const result = await service.remindersSnooze(params, runtime); if (FIRST_LIVE_FEATURES.notifications) await service.notificationReconcile?.(runtime); return result; },
  'command-center.v1.reminders.complete': async (service, params, runtime) => { const result = await service.remindersComplete(params, runtime); if (FIRST_LIVE_FEATURES.notifications) await service.notificationReconcile?.(runtime); return result; },
