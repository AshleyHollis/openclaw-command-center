...
export const READ_METHODS = Object.freeze([
  ...
  'command-center.v1.sessions.topic-context',
  'command-center.v1.sessions.group-preview',
...
export const WRITE_METHODS = Object.freeze([
  ...
  'command-center.v1.sessions.group',
...
function parameterSchema(field, method) {
  ...
  if (field === 'outcome') return Object.freeze({ type: 'string', enum: ['clear', 'ambiguous'] });
...
function actionResultSchema(method) {
  ...
  if (method.startsWith('command-center.v1.open-loops.')) {
    const supportingNote = Object.freeze({ type: 'object', additionalProperties: false,
      properties: Object.freeze({ status: { enum: ['pending', 'completed', 'unknown', 'conflict', 'unavailable', 'superseded'] },
        logicalOperationId: { type: 'string' }, reason: { type: 'string' }, priorDecision: { type: 'boolean' } }), required: ['status'] });
    const attention = Object.freeze({ type: 'object', additionalProperties: false, properties: Object.freeze({ reason: { type: 'string' }, whyNow: { type: 'string' }, actions: { type: 'array', items: { type: 'string' } }, materialRevision: { type: 'string' }, pendingClarificationId: { type: 'string' }, activated: { type: 'boolean' }, currentEvidence: { type: 'boolean' }, importance: { type: 'string' }, importanceOrigin: { type: 'string' }, plannedAt: { type: 'string' }, effortMinutes: { type: 'integer' }, contexts: { type: 'array', items: { type: 'string' } }, dependencies: { type: 'array', items: { type: 'string' } }, provenance: { type: 'string' }, confidence: { type: 'number' }, lastConsideredAt: { type: 'string' }, someday: { type: 'boolean' } }) });
    ...
    if (method.endsWith('.get')) {
      const followUp = Object.freeze({ type: 'object', additionalProperties: false, properties: Object.freeze({ status: { enum: ['pending', 'completed', 'unknown', 'conflict', 'blocked', 'unavailable'] }, logicalOperationId: { type: 'string' }, action: { enum: ['create', 'reschedule', 'cancel', 'none', 'blocked', 'conflict'] }, reason: { type: 'string' }, priorDecision: { type: 'boolean' } }), required: ['status', 'logicalOperationId'] });
      const interpretation = Object.freeze({ type: 'object', additionalProperties: false, properties: Object.freeze({ status: { enum: ['pending', 'proposal-saved', 'review-required', 'processing-failed'] } }), required: ['status'] });
      return Object.freeze({ type: 'object', additionalProperties: false, properties: Object.freeze({ schemaVersion: { const: 1 }, loop, evidence: { type: 'array', items: evidence }, interpretation, followUp, supportingNote }), required: ['schemaVersion', 'loop', 'evidence'] });
    }
    ...
    if (method.endsWith('.interpret-clarification')) return Object.freeze({ type: 'object', additionalProperties: false,
      properties: Object.freeze({ schemaVersion: { const: 1 }, status: { type: 'string' }, reason: { type: 'string' }, loopId: { type: 'string' }, disposition: { enum: ['applied', 'duplicate'] }, loop, reminder, supportingNote }), required: ['schemaVersion'] });
  ...
  if (method === 'command-center.v1.sessions.group-preview') return Object.freeze({ type: 'object', additionalProperties: false, properties: {
    ...
    members: { type: 'array', items: { type: 'object', additionalProperties: false, properties: {
      referenceId: { type: 'string' }, sessionId: { type: 'string' }, lifecycleRevision: { type: ['string', 'null'] }, grouped: { type: 'boolean' }, label: { type: 'string' }, eligible: { type: 'boolean' }
    } } }
  } });
  ...
  if (method === 'command-center.v1.sessions.topic-context') return Object.freeze({
    type: 'object', additionalProperties: false,
    properties: Object.freeze({ schemaVersion: { const: 1 }, status: { enum: ['bound', 'unbound', 'protected', 'protected-unavailable'] }, sessionKey: { type: 'string' }, sessionId: { type: 'string' }, topicId: { type: ['string', 'null'] }, referenceId: { type: 'string' }, name: { type: 'string' } }),
    required: ['schemaVersion', 'status', 'sessionKey']
  });
  if (method === 'command-center.v1.notes.browse') {
    const sourceReference = Object.freeze({ type: 'object' });
    const note = Object.freeze({ type: 'object', additionalProperties: false, properties: Object.freeze({ schemaVersion: { const: 1 }, path: { type: 'string' }, revision: { type: 'string' }, sourceKind: { enum: ['note', 'document'] }, sourceReference }), required: ['schemaVersion', 'path', 'revision', 'sourceKind', 'sourceReference'] });
  ...
  const properties = {
    ...
    topicRevision: Object.freeze({ type: 'integer' }),
  ...
  if (method === 'command-center.v1.sessions.assign-topic') return Object.freeze({
    ...
    properties: Object.freeze({
      ...
      status: Object.freeze({ type: 'string', enum: ['applied', 'replayed'] }),
      logicalOperationId: Object.freeze({ type: 'string', minLength: 1 }),
      topicId: properties.topicId,
      referenceId: properties.referenceId,
      sessionKey: properties.sessionKey,
      sessionId: properties.sessionId,
      topicRevision: properties.topicRevision
    }),
    required: Object.freeze(['schemaVersion', 'status', 'logicalOperationId', 'topicId', 'referenceId', 'sessionKey', 'sessionId', 'topicRevision'])
  });
  ...
  const allowed = method.includes('.migration.')
    ...
    : method.endsWith('sessions.topic-context')
    ? ['schemaVersion', 'status', 'sessionKey', 'sessionId', 'topicId', 'referenceId', 'name']
    : method.endsWith('schedules.get')
    ? ['schemaVersion', 'sourceReference', 'job']
    : method.endsWith('metadata.read')
    ? ['schemaVersion', 'topic', 'topics', 'sourceReferences', 'preferences', 'activity', 'maintenance', 'state', 'version', 'referenceId', 'topicId', 'sourceSystem', 'sourceKind', 'externalSourceId', 'observedRevision', 'createdAt', 'updatedAt']
...
const required = Object.freeze({
  ...
  'command-center.v1.sessions.topic-context': ['sessionKey'],
  'command-center.v1.sessions.group-preview': ['topicId'],
  'command-center.v1.sessions.group': ['topicId', 'referenceId', 'expectedSessionId', 'expectedLifecycleRevision', 'expectedTopicRevision', 'name'],
  'command-center.v1.sessions.assign-topic': ['topicId', 'sessionKey', 'expectedSessionId', 'expectedSessionRevision', 'expectedMembership', 'expectedTopicRevision'],
  ...
  'command-center.v1.open-loops.clarify': ['loopId', 'expectedRevision', 'rationale'],
  'command-center.v1.open-loops.interpret-clarification': ['loopId', 'expectedRevision', 'clarificationObservationId', 'processorVersion', 'outcome'],
  ...
  'command-center.v1.open-loops.renovation-purchase-correction': ['expectedRevision', 'correction'],
...
const fields = Object.freeze({
  ...
  'command-center.v1.topics.recovery.replace': ['topicId', 'referenceId', 'replacementLocator', 'sessionKey', 'sessionId', 'expectedRevision', 'expectedSourceRevision'],
  'command-center.v1.sources.status': [],
  'command-center.v1.notes.browse': ['topicId', 'limit', 'offset', 'cursor', 'includeDocuments'],
  'command-center.v1.notes.read': ['topicId', 'referenceId', 'path', 'notePath', 'offset', 'observedRevision', 'sourceKind'],
  ...
  'command-center.v1.sessions.group': ['topicId', 'referenceId', 'expectedSessionId', 'expectedLifecycleRevision', 'expectedTopicRevision', 'name'],
  'command-center.v1.sessions.assign-topic': ['topicId', 'sessionKey', 'expectedSessionId', 'expectedSessionRevision', 'expectedMembership', 'expectedTopicRevision'],
  'command-center.v1.sessions.create': ['topicId', 'label', 'isPrimary', 'expectedRevision', 'logicalOperationId', 'authoritativeSession'],
  ...
  'command-center.v1.open-loops.clarify': ['loopId', 'expectedRevision', 'rationale', 'logicalOperationId'],
  'command-center.v1.open-loops.interpret-clarification': ['loopId', 'expectedRevision', 'clarificationObservationId', 'processorVersion', 'outcome', 'decision', 'paymentState', 'paidAmount', 'currency', 'reviewAt', 'dueAt', 'dueDate', 'dueTimeZone'],
  'command-center.v1.open-loops.resume-follow-up': ['logicalOperationId'],
  ...
  'command-center.v1.open-loops.renovation-purchase-correction': ['expectedRevision', 'correction', 'logicalOperationId'],
...
export function validateBridgeRequest(method, params, { mutation = WRITE_METHODS.includes(method) } = {}) {
  ...
  if (method.includes('.sessions.') && !method.endsWith('.create') && !method.endsWith('.browse') && !method.endsWith('.topic-context') && !method.endsWith('.group-preview') && !method.endsWith('.assign-topic') && !(typeof params.referenceId === 'string' || typeof params.sessionReferenceId === 'string')) throw sourceError('invalid-request', 'Bridge Session request requires an exact Source Reference.');
  ...
  if (method.endsWith('.open-loops.decide') && (params.decision === 'defer') !== (params.reviewAt !== undefined)) throw sourceError('invalid-request', 'Only a deferred open loop requires reviewAt.');
  if (method.endsWith('.open-loops.decide')) {
    const dateOnly = params.dueDate !== undefined || params.dueTimeZone !== undefined;
    if ((params.dueDate === undefined) !== (params.dueTimeZone === undefined) || params.dueAt !== undefined && dateOnly) throw sourceError('invalid-request', 'Corrected timing must be one instant or one calendar date with timezone.');
    const correctedTiming = params.dueAt !== undefined || dateOnly;
    if (correctedTiming && !['correct-date', 'confirm'].includes(params.decision) || params.decision === 'correct-date' && !correctedTiming) throw sourceError('invalid-request', 'corrected timing is required for correct-date and optional while confirming a suggestion.');
    if ((params.amount === undefined) !== (params.currency === undefined) || params.amount !== undefined && params.decision !== 'confirm') throw sourceError('invalid-request', 'Corrected amount and currency are only accepted together while confirming a suggestion.');
  }
  if (method.endsWith('.open-loops.payment-status') && ((params.paidAmount === undefined) !== (params.currency === undefined))) throw sourceError('invalid-request', 'paidAmount and currency must be provided together.');
  if (method.endsWith('.open-loops.interpret-clarification')) {
    const decision = params.decision !== undefined;
    const payment = params.paymentState !== undefined;
    if (params.outcome === 'clear' && decision === payment || params.outcome === 'ambiguous' && (decision || payment)) throw sourceError('invalid-request', 'Interpretation requires exactly one clear decision or payment status, or an ambiguous result without either.');
    if ((params.paidAmount === undefined) !== (params.currency === undefined) || params.paidAmount !== undefined && !payment) throw sourceError('invalid-request', 'Interpreted payment amount and currency must accompany a payment status.');
    const dateOnly = params.dueDate !== undefined || params.dueTimeZone !== undefined;
    if ((params.dueDate === undefined) !== (params.dueTimeZone === undefined) || params.dueAt !== undefined && dateOnly) throw sourceError('invalid-request', 'Interpreted timing must be one instant or one calendar date with timezone.');
  }
  if (method.endsWith('.open-loops.organize')) {
    if ((params.action === 'plan') !== (params.plannedAt !== undefined)) throw sourceError('invalid-request', 'Only plan requires plannedAt.');
...
function sanitizeNote(value) {
  const result = copyClosed(value, ['schemaVersion', 'path', 'text', 'revision', 'sourceKind', 'sourceReference'], 'Note');
