...
function parameterSchema(field, method) {
  ...
  if (field === 'paidAmount') return Object.freeze({ type: 'integer', minimum: 0 });
...
const fields = Object.freeze({
  ...
  'command-center.v1.open-loops.decide': ['loopId', 'expectedRevision', 'decision', 'reviewAt', 'dueAt', 'dueDate', 'dueTimeZone', 'rationale', 'logicalOperationId'],
...
export function validateBridgeRequest(method, params, { mutation = WRITE_METHODS.includes(method) } = {}) {
  ...
  if (method.endsWith('.open-loops.decide')) {
    ...
    if ((params.decision === 'correct-date') !== (params.dueAt !== undefined || dateOnly)) throw sourceError('invalid-request', 'Only a corrected-date decision requires corrected timing.');
  }
