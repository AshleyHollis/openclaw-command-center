...
export function briefingPublishToolFactory({ getOwner, resolveSessionKey } = {}) {
  return (context = {}) => ({
    ...
    async execute(toolCallId, params) {
      ...
      const sessionKey = context.sessionKey ?? await resolveSessionKey?.(context);
      if (!owner || !sessionKey) throw new Error('Briefing publication requires an active native report session.');
      const value = owner.publishBriefing({ schemaVersion: 1, logicalOperationId: operationId({ ...context, sessionKey }, toolCallId), ...params, source: { kind: 'session', sessionKey } });
