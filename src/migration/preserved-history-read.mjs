...
export async function createPreservedHistoryReader(options) {
  ...
  const inventory = await readDiscordPreservationInventory(structuredClone(options.sourceOptions));
  ...
  const descriptor = (receipt, source) => ({ historyId: receipt.historyId, topicId: receipt.intent.topicId,
    title: source.sourceChannel.name ?? 'Imported History', totalMessages: receipt.verifiedCount, readOnly: true });
  ...
  return Object.freeze({
    async list(input, check) {
      ...
      for (const row of metadata.listImportedHistories()) {
        if (row.phase !== 'verified' || row.intent.sourceManifestSha256 !== inventory.bundle.manifestSha256 || (input.topicId !== undefined && row.intent.topicId !== input.topicId)) continue;
    ...
    async read(input, check) {
      ...
      for (const entry of entries.slice(offset, offset + limit)) {
        ...
        const message = { messageId: entry.entryId, author: entry.message.__openclaw.senderName, bot: original.author.bot === true,
      ...
      return { schemaVersion: 1, ...descriptor(receipt, prepared.get(receipt.intent.sourceChannelId)), messages, offset,
