...
// Internal execution owner: caller supplies a signed-source-admitted, frozen
...
async function run(options, readOnly) {
  ...
  if (!row || !prepared || !Array.isArray(prepared.entries) || prepared.sourceChannel?.id !== prepared.sourceChannelId || row.intent.sourceDigest !== digest({ channel: prepared.sourceChannel, entries: prepared.entries }) || row.intent.sourceDigest !== prepared.sourceDigest || row.intent.sourceChannelId !== prepared.sourceChannelId || row.intent.expectedCount !== prepared.expectedCount || prepared.entries.length !== prepared.expectedCount) fail('history-source-conflict');
