...
import { prepareDiscordPreservation, importDiscordPreservation } from './preserved-history-batch.mjs';
...
export async function reconcilePreservedWorkspace(options) {
  ...
  if (!exact(plan, ['schemaVersion', 'logicalOperationId', 'sourceOptions', 'bootstraps', 'mappings', 'protectedSessions']) || plan.schemaVersion !== 1 ||
  ...
  const histories = source.selected.map(item => metadata.inspectImportedHistory({ logicalOperationId: item.mapping.logicalOperationId, intent: item.intent }, checkSources));
  ...
  const reservationInput = { logicalOperationId: plan.logicalOperationId, planDigest: expectedPlanDigest, children: [
    ...
    ...histories.map(item => ({ logicalOperationId: item.logicalOperationId, operationKind: 'history.import.v1', intentDigest: item.intentDigest }))
  ] };
  ...
  if (mode === 'preflight') return freeze({ phase: 'preflight', planDigest: expectedPlanDigest, children: sortedChildren,
    accounting: { topics: bootstraps.length, sourceChannels: source.counts.channels, sourceMessages: source.counts.messages, sourceAttachments: source.counts.attachments }, attachmentCoverage: source.attachmentCoverage });
