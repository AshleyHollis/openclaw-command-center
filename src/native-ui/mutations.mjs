...
async function nativeResponse(host, path, input, signal) {
  ...
  // The Control UI asset grant is not write authority. The authenticated HTTP
  // route performs the operator/scope check at the mutation owner boundary.
  if (!host.connection.connected || !host.connection.canRead || typeof host.httpRequest !== 'function') throw new Error('Connect with authenticated native HTTP support to make changes.');
