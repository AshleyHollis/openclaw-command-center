...
export function mountAttentionPage(container, context, operations = new Map(), pageMode = 'dashboard') {
  ...
  function renderEvidence(disclosure, detail) {
    ...
    if (nonBlank(loop?.attention?.pendingClarificationId)) {
      ...
      disclosure.append(element('p', wording));
    ...
    if (followUp) {
      ...
      if (!followUp.priorDecision && (followUp.status === 'pending' || ['pending', 'unknown', 'unavailable'].includes(supportingNote?.status)) && nonBlank(followUp.logicalOperationId)) {
