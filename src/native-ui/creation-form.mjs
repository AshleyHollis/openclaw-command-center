...
export function createNativeCreationForm({ host, state, document, signal, presented, getTopic, onCreated, beginNavigation = () => () => true }) {
  ...
  const allowed = () => state.active && !disposed && !signal.aborted && !host.signal?.aborted && presented() && host.connection.connected && host.connection.canRead && host.connection.canWrite && typeof host.httpRequest === 'function' && (!conversation || getTopic()?.topicId === topicId && getTopic()?.usable === true && getTopic()?.lifecycle === 'active');
  ...
  function sync() {
    if (!allowed()) { if (conversation) inspection = 'unavailable'; request.abort(); request = new AbortController(); }
    ...
    else if (!allowed()) status.textContent = `Connect with write access and an available ${conversation ? 'Topic' : 'native HTTP connection'} to create a ${kind}.`;
  ...
  async function recover(action) {
    ...
    const pendingSignal = AbortSignal.any([signal, request.signal, ...(host.signal ? [host.signal] : [])]);
    const owns = () => state.active && recoveryAttempt === attempt && operations.get(key) === operation;
    const interrupted = () => {
      if (recoveryAttempt !== attempt) return;
      recoveryAttempt = null; inspection = 'unavailable';
    ...
    try {
      ...
      if (receipt.status === 'clear' || receipt.status === 'blocked') {
        ...
        inspection = receipt.status;
      } else if (receipt.status === 'acknowledged') {
        operations.delete(key); inspection = 'clear'; form.reset();
      } else {
        ...
        operations.set(key, retained); name.value = recoveredInput.label ?? receipt.result.label ?? ''; inspection = 'clear';
      }
    } catch (error) {
      if (!owns() || pendingSignal.aborted || !allowed()) return;
      inspection = 'unavailable'; recoveryError = host.redact(error.message);
    } finally {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const submissionSignal = AbortSignal.any([signal, request.signal]);
    ...
    try {
      const result = await nativeMutation(host, `/plugins/command-center/api/${conversation ? 'topic' : 'topics'}/actions`, input, submissionSignal);
      ...
      if (conversation ? result?.action !== input.action || result?.topicId !== input.topicId || typeof result?.referenceId !== 'string' || !result.referenceId : result?.value?.topicId !== input.topicId || result?.value?.status !== 'applied') {
        throw Object.assign(new Error('The creation receipt is incomplete. Source Recovery is required.'), { unknownOutcome: true });
      }
      if (conversation) { operation.result = result; operation.unknown = false; operation.attempt = null; }
      ...
      try { await onCreated(result, input); }
      ...
      if (!owns()) return;
      if (error.unknownOutcome || submissionSignal.aborted) { operation.unknown = true; operation.attempt = null; }
