...
export function normalizeEmailReaderPlan(input) {
  ...
  const records = input.records.map(record => {
    if (!record || typeof record !== 'object' || Array.isArray(record) || Object.keys(record).some(key => !['sourceExternalId', 'sourceVersion', 'messageId', 'webLink', 'status', 'observedAt'].includes(key)) || !['available', 'unavailable'].includes(record.status)) fail();
    let webLink;
    if (record.status === 'available') {
      try { webLink = validatedOutlookWebLink(record.webLink); } catch { fail(); }
    } else if (record.webLink !== undefined) fail();
    const observedAt = text(record.observedAt, 64);
    if (!Number.isFinite(Date.parse(observedAt))) fail();
    return Object.freeze({ sourceExternalId: text(record.sourceExternalId, 1000), sourceVersion: text(record.sourceVersion, 300), messageId: text(record.messageId, 1000), status: record.status, ...(webLink ? { webLink } : {}), observedAt: new Date(observedAt).toISOString() });
  });
