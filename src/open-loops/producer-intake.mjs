...
export function createProducerIntakeAdapter({ processorVersion, extract, loadIntakeSourceAccount, resolveTopic, saveSourceNote, captureSourceCommitment, captureChatCommitment, recordIntakeSourcePlan, recordIntakeOutcome, recordIntakeReceipt, now = () => new Date().toISOString() } = {}) {
  ...
  return Object.freeze({
    async process({ runId, sourceKind: selectedSourceKind, records, nextExpectedAt, enumeration, scope, purpose = 'producer', retryOfRunId, planDigest }) {
      ...
      if (!['producer', 'admitted-retry'].includes(purpose) || purpose === 'admitted-retry' && (!nonBlank(retryOfRunId) || !/^sha256:[a-f0-9]{64}$/u.test(planDigest) || enumeration !== undefined || selectedSourceKind !== 'email') || purpose === 'producer' && retryOfRunId !== undefined || planDigest !== undefined && !/^sha256:[a-f0-9]{64}$/u.test(planDigest)) fail('producer-batch-invalid');
      ...
      const receiptContext = { ...(purpose === 'admitted-retry' ? { purpose, retryOfRunId } : {}), ...(planDigest ? { planDigest } : {}), ...(scope ? { scope } : {}), ...(receiptEnumeration ? { enumeration: { scope: receiptEnumeration.scope, scannedCount: receiptEnumeration.scannedCount, remainingCount: receiptEnumeration.remainingCount, failedReadCount: receiptEnumeration.failedReadCount, scanCapReached: receiptEnumeration.scanCapReached } } : {}) };
