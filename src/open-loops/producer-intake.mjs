...
function unfinished(account, outcomeId) { return !accountOutcome(account, outcomeId) || ['missing', 'unresolved-topic', 'failed', 'unknown'].includes(accountOutcome(account, outcomeId).status); }
...
export function createProducerIntakeAdapter({ processorVersion, extract, loadIntakeSourceAccount, resolveTopic, saveSourceNote, captureSourceCommitment, captureChatCommitment, recordIntakeSourcePlan, recordIntakeOutcome, recordIntakeReceipt, now = () => new Date().toISOString() } = {}) {
  ...
  return Object.freeze({
    async process({ runId, sourceKind: selectedSourceKind, records, nextExpectedAt, enumeration, scope, purpose = 'producer', retryOfRunId, planDigest, unadmittedSourceCount }) {
      ...
      if (!['producer', 'admitted-retry'].includes(purpose) || purpose === 'admitted-retry' && (!nonBlank(retryOfRunId) || !/^sha256:[a-f0-9]{64}$/u.test(planDigest) || enumeration !== undefined || selectedSourceKind !== 'email') || purpose === 'producer' && (retryOfRunId !== undefined || unadmittedSourceCount !== undefined) || unadmittedSourceCount !== undefined && (!Number.isSafeInteger(unadmittedSourceCount) || unadmittedSourceCount < 0) || planDigest !== undefined && !/^sha256:[a-f0-9]{64}$/u.test(planDigest)) fail('producer-batch-invalid');
      ...
      const receiptContext = { ...(purpose === 'admitted-retry' ? { purpose, retryOfRunId, ...(unadmittedSourceCount === undefined ? {} : { unadmittedSourceCount }) } : {}), ...(planDigest ? { planDigest } : {}), ...(scope ? { scope } : {}), ...(receiptEnumeration ? { enumeration: { scope: receiptEnumeration.scope, scannedCount: receiptEnumeration.scannedCount, remainingCount: receiptEnumeration.remainingCount, failedReadCount: receiptEnumeration.failedReadCount, scanCapReached: receiptEnumeration.scanCapReached } } : {}) };
      ...
      try {
        ...
        const receipt = await recordIntakeReceipt({ sourceKind, runId, checkpoint, status, observedAt: completedAt, ...(status === 'incomplete' ? { continuation } : { lastSuccessfulAt: completedAt }), nextExpectedAt, ...receiptContext, ...receiptCounts() });
        ...
        await recordIntakeReceipt({ sourceKind, runId, checkpoint, status: 'failed', observedAt: now(), ...(continuation ? { continuation } : {}), ...receiptContext, ...receiptCounts() });
