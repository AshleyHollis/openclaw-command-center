export const optionalCapabilities = Object.freeze(['notes', 'sessions', 'scheduler']);

const capabilityLabels = Object.freeze({ notes: 'Notes', sessions: 'Sessions', scheduler: 'scheduler' });
...
export function evaluateOperatingMode({ core, capabilities }) {
  ...
  if (core.mode !== 'ready' || core.schemaVersion !== 1) {
    ...
    return Object.freeze({
      ...
      diagnostics: Object.freeze([Object.freeze({
        ...
        remediation: 'Restart with a supported schema-1 core store before changing metadata.'
      })]),
      unavailableCapabilities: Object.freeze([])
    });
  }
  const unavailable = optionalCapabilities.filter((capability) => capabilities[capability]?.available === false);
