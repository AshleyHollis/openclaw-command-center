...
import { IMPORTED_HISTORY_OPERATION, installImportedHistoryMetadata } from './imported-history.mjs';
...
function createService(stateDir, databasePath, capabilities, migrationHooks, readOnly = false) {
  ...
  service.recordOperation = (input) => {
    ...
    return mutate(null, (db) => {
      ...
      if (operationKind === IMPORTED_HISTORY_OPERATION || existing?.operation_kind === IMPORTED_HISTORY_OPERATION) throw new CommandCenterMetadataError('history-owner-required', 'Imported History receipts require their dedicated owner.');
