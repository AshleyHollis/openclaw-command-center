...
document.querySelector('#topic-search-rebuild')?.addEventListener('click', async () => { const status = document.querySelector('#topic-search-status'); status.textContent = 'Rebuilding Topic Search…'; try { const response = await fetch(SEARCH_REBUILD_ROUTE, { method: 'POST', credentials: 'omit', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ schemaVersion: 1, topicId: document.querySelector('#topic-search-topic-id').value, logicalOperationId: operationId() }) }); const value = await response.json(); if (!response.ok || value.status === 'error') throw new Error(value.message || 'Topic Search rebuild was refused.'); status.textContent = 'Topic Search index rebuilt from authoritative sources.'; } catch (error) { status.textContent = error.message || 'Topic Search rebuild failed.'; } });
...
async function rebuildWorkspaceSearch() { if (!workspace.topic) return; const topicId = workspace.topic.topicId; const generation = workspace.generation; workspaceSearchStatus.textContent = 'Rebuilding Topic Search…'; try { const response = await fetch(SEARCH_REBUILD_ROUTE, { method: 'POST', credentials: 'omit', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ schemaVersion: 1, topicId, logicalOperationId: operationId() }) }); const value = await response.json(); if (!response.ok || value.status === 'error') throw new Error(value.message || 'Topic Search rebuild was refused.'); if (generation === workspace.generation && workspace.topic?.topicId === topicId) workspaceSearchStatus.textContent = 'Topic Search index rebuilt from authoritative sources.'; } catch (error) { if (generation === workspace.generation && workspace.topic?.topicId === topicId) workspaceSearchStatus.textContent = error.message || 'Topic Search rebuild failed.'; } }
