...
First qualify the targeted native handoff after resolving the host contract gap. Then replace the complete journey's removed embedded-composer interaction with the qualified round trip, carrying the remounted frame through its callers. Preserve actual native Session message readback, keyboard/mobile checks and measured Chat-send performance; do not replace these with a navigation-only pass or a mocked host result.
