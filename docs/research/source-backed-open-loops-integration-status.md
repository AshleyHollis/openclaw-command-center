...
| Selected document | Plugin integration uses an authoritative reader fixture. A bounded extraction corpus measures six supported layouts against six negative/adversarial documents. A composed descriptor-backed owner → plugin → Reminder → restart/outage test is authored and explicitly skipped on Windows. | Implemented plaintext pilot, pending Linux execution and packaged real-owner proof. |
